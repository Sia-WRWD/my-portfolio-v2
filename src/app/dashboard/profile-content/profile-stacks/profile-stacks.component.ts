...
export class ProfileStacksComponent {
  ...
  ngOnInit() {
    // Initially, show all languages
    this.filteredStacks = this.flattenStacks(stacks);
    console.log(this.filteredStacks);

    // Set the initial filter state
    this.filters.forEach((filter: any) => {
      if (filter.shorthand === "WebDev") {
        filter.state = "Active";
      } else {
    ...
    // Filter the stacks to show only Web Dev initially
    const webDevFilter = this.filteredStacks.find((filter: any) => filter.shorthand === "WebDev");
    if (webDevFilter) {
      const filteredCategory = stacks.find((stack: any) => stack.shorthand === "WebDev");
      this.filteredStacks = [filteredCategory];
    }
  ...
  flattenStacks(stackData: any[]): any[] {
    return stackData.reduce((acc, category) => {
      return acc.concat(category.languages.map((language: any) => ({ category: category.category, ...language })));
    }, []);
  }
  ...
  changeStack(filterValue: string) {
    ...
    if (filterValue === "All") {
      // If 'All' is selected, show all languages
      this.filteredStacks = this.flattenStacks(stacks);
    } else {
      // Filter the languages based on the selected category
      this.filteredStacks = this.flattenStacks(stacks.filter((stack: any) => stack.shorthand === filterValue));
    }
  }
