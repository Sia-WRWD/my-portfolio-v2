...
export class ProfileStacksComponent {
  ...
  ngOnInit() {
    ...
    const webDevFilter = this.filters.find((filter: any) => filter.shorthand === "WebDev");
    if (webDevFilter) {
  ...
  changeStack(filterValue: string) {
    ...
    if (filterValue === "All") {
      // If 'All' is selected, show all stacks
      ...
      // Set filteredStacks to contain only the selected category
      this.filteredStacks = [filteredCategory];
    }
  }
